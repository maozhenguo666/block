...
class Feeder(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=1, split='train', random_choose=False, random_shift=False,
                 random_move=False, random_rot=False, window_size:int=64, normalization=False, debug=False, use_mmap=False,
    ...
    def __getitem__(self, index)-> (torch.Tensor, torch.Tensor):
        ...
        if(valid_frame_num == 0): 
            return  np.zeros((3, 64, 17, 2)), np.zeros((3, 64, 17, 2)), label, index
...
#             data_numpy[:, :, 20] = trajectory
        ...
        if self.vel:
            ...
            data_numpy[:, -1] = 0
        # print("data_numpy 的形状:", data_numpy.shape)
        # print("joint 的形状:", joint.shape)
