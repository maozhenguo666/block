...
class Tem_Seq_h(nn.Module):
    ...
    def forward(self, x):    
        x = self.conv(x)
        N,C,T,V = x.size()
        x = x.permute(0,2,1,3).contiguous()  #(N, T, C, M)
...
class Tem_Trans(nn.Module):
    ...
    def forward(self, x):    
        ...
        V_Tem_Trans = self.MaxTemTrans(x3.permute(0,2,1,3).contiguous())
        ##
        ...
        V_Tem_Trans = self.relu(self.linear(V_Tem_Trans.squeeze(-1).squeeze(-1)))
        ##       
        Tem_atten_QK = self.sigmoid(torch.einsum('nt,nm->ntm', (Q_Tem_Trans, K_Tem_Trans)))
        Tem_atten_KV = self.sigmoid(torch.einsum('nt,nm->ntm', (K_Tem_Trans, V_Tem_Trans)))
        Tem_atten_QV = self.sigmoid(torch.einsum('nt,nm->ntm', (Q_Tem_Trans, V_Tem_Trans)))
        ##
        Tem_atten = (Tem_atten_QK + Tem_atten_KV + Tem_atten_QV) / 3
