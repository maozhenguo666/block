...
class Model(nn.Module):
    ...
    def forward(self, x):
        N, C, T, V, M = x.size()
        # print("0Shape of x before data_bn:", x.shape)
        x = rearrange(x, 'n c t v m -> (n m t) v c', m=M, v=V).contiguous()
        # print("000Shape of x before data_bn:", x.shape)
        ...
        # print("1Shape of x before data_bn:", x.shape)
        x = self.data_bn(x)
        x = rearrange(x, 'n (m v c) t -> (n m) c t v', m=M, v=V).contiguous()
        # print("2Shape of x before data_bn:", x.shape)
