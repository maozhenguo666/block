...
def get_parser():
    ...
    # visulize and debug
    parser.add_argument('--save_epoch', type=int, default=1, help='the start epoch to save model (#iteration)')
...
class Processor():
    ...
    def train(self, epoch, save_model=True):
        ...
        if  save_model:
            ...
            torch.save(weights, self.arg.work_dir + '/' + self.arg.work_dir.split('/')[-1] + '-' + str(epoch+1) + '-' + '.pt')
